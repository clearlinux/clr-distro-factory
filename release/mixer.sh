...
get_upstream_prev_info() {
    # First check if we have a local, unpublished Mix
    if [ -d "update/${MIX_LATEST_VERSION}" ]; then
        { # Get the Upstream version and format for the previous mix
            UPSTREAM_PREV_VERSION=$(cat update/${MIX_LATEST_VERSION}/upstream_ver) &&
            UPSTREAM_PREV_FORMAT=$(curl --silent --fail ${UPSTREAM_URL}update/${UPSTREAM_PREV_VERSION}/format)
        } || { # Failed
            echo "[local] Failed to get Upstream previous ClearLinux version information!"
            exit 2
        }
    else
        { # Get the Upstream version and format for the previous mix
            UPSTREAM_PREV_VERSION=$(curl --silent --fail file://${STAGING_DIR}/update/${MIX_LATEST_VERSION}/upstream_ver) &&
...
get_latest_versions() {
    ...
    if [ -z ${MIX_LATEST_VERSION} ]; then
        # This is the First Mix, fallback to the Public upstream
        echo "INFO: No latest Mix, assume this is the first Mix..."

        UPSTREAM_URL="${CLR_PUBLIC_DL_URL}/update/"
        {
            ...
            echo "Failed to get ClearLinux (public) upstream latest version information!"
            exit 4
        }
        ...
        { # Get Latest Mix version's format (we already have the version)
            ...
            echo "Failed to get Latest Mix ClearLinux version information!"
            exit 2
        }

        {
            UPSTREAM_URL=${UPSTREAM_URL:-$(curl --silent --fail file://${STAGING_DIR}/update/${MIX_LATEST_VERSION}/upstream_url)} &&
            UPSTREAM_BASE_VERSION=${UPSTREAM_BASE_VERSION:-$(curl --silent --fail ${UPSTREAM_URL}../latest)} &&
            UPSTREAM_BASE_FORMAT=$(curl --silent --fail ${UPSTREAM_URL}${UPSTREAM_BASE_VERSION}/format)
        } || { # Failed
            echo "Failed to get ClearLinux upstream version information!"
            exit 4
        }
